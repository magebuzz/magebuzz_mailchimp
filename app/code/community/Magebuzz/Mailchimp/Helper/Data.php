...
class Magebuzz_Mailchimp_Helper_Data extends Mage_Core_Helper_Abstract {
  ...
  public function filterShowGroupings($interestGroupings) {
    if (is_array($interestGroupings)) {
      $customGroupings = (array)Mage::getConfig()->getNode('default/mailchimp/custom_groupings');
  ...
  public function getCustomerGroupingName() {
    return (string)Mage::getConfig()->getNode('default/mailchimp/custom_groupings/customer_grouping_name');
  }
  ...
  public function getUserAgent() {
    ...
    $v = (string)Mage::getConfig()->getNode('modules/Magebuzz_Mailchimp/version');
    $version = strpos(Mage::getVersion(), '-') ? substr(Mage::getVersion(), 0, strpos(Mage::getVersion(), '-')) : Mage::getVersion();
    return (string)'Magebuzz_Mailchimp' . $v . '/Mage' . $aux . $version;
  }
   ...
   * @param string $filename log filename, default is <MailChimp.log>
   * @return Mage_Core_Model_Log_Adapter
   */
  public function log($data, $filename = 'MailChimp.log') {
  ...
  public function config($value, $store = null) {
    ...
    return Mage::getStoreConfig("mailchimp/general/$value", $store);
  }
  ...
  public function subscribedToList($email, $listId = null) {
    $on = FALSE;
    if ($email) {
      $member = Mage::getSingleton('mailchimp/api')->listMemberInfo($listId, $email);
  ...
  public function useTransactionalService() {
    return Mage::getStoreConfigFlag("mailchimp/general/transactional_emails");
  }

  /**
   * Check if Ebizmarts_Magebuzz_Mailchimp module is enabled
   *
   * @return bool
   */
  ...
  public function getStoreByList($mcListId, $includeDefault = FALSE) {
    $list = Mage::getModel('core/config_data')->getCollection()->addValueFilter($mcListId)->getFirstItem();
  ...
  public function getMergeVars($customer, $includeEmail = FALSE, $websiteId = NULL) {
    ...
    if ($customer->getCustomerId()) {
      $customer->addData(Mage::getModel('customer/customer')->load($customer->getCustomerId())->toArray());
    }
    ...
    //Magebuzz_Mailchimp_mergevars_after
  ...
  public function createCustomerAccount($accountData, $websiteId) {
    ...
    $customerForm->setFormCode('customer_account_create')->setEntity($customer)->initDefaultValues();
    // emulate request
  ...
  public function handlePost($request, $guestEmail) {
    ...
    $api = Mage::getSingleton('mailchimp/api');
      ...
      foreach ($curlists as $listId => $list) {
        ...
        if (FALSE === array_key_exists($listId, $lists)) {
          ...
          if ($defaultList == $listId) {
            ...
            $item = Mage::getModel('mailchimp/mailchimp')->loadByEmail($email);
            if (!$item->getId()) {
          ...
          $mergeVars = Mage::helper('mailchimp')->getMergeVars($customer);
      ...
      if (!empty($subscribe)) {
        ...
        foreach ($subscribe as $listId => $slist) {
          ...
          if ($defaultList == $listId) {
            ...
            if (!Mage::helper('mailchimp')->isAdmin() && (Mage::getStoreConfig(Mage_Newsletter_Model_Subscriber::XML_PATH_CONFIRMATION_FLAG, Mage::app()->getStore()->getId()) == 1)) {
              $isConfirmNeed = TRUE;
            }
            $customer->setListGroups($groupings);
            $customer->setMcListId($listId);
