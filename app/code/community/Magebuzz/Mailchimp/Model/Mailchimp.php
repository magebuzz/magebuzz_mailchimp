...
class Magebuzz_Mailchimp_Model_Mailchimp {
  ...
  protected function _updateEmail(array $data) {
    ...
    if (!$newSubscriber->getId() && $oldSubscriber->getId()) {
      $oldSubscriber->setSubscriberEmail($new)->save();
    } elseif (!$newSubscriber->getId() && !$oldSubscriber->getId()) {

      Mage::getModel('newsletter/subscriber')->setImportMode(TRUE)->setStoreId(Mage::app()->getStore()->getId())->subscribe($new);
    }
  }
  ...
  protected function _clean(array $data) {
    ...
    $this->_getInbox()->setTitle($text)->setDescription($text)->save();
  ...
  protected function _subscribe(array $data) {
    try {
      ...
      //TODO: El m�todo subscribe de Subscriber (Magento) hace un load by email
      ...
      $subscriber = Mage::getModel('newsletter/subscriber')->loadByEmail($data['data']['email']);
      if ($subscriber->getId()) {
  ...
  protected function _getInbox() {
    return Mage::getModel('adminnotification/inbox')->setSeverity(4)//Notice
  ...
  public function loadByEmail($email) {
    return Mage::getModel('newsletter/subscriber')->getCollection()->addFieldToFilter('subscriber_email', $email)->addFieldToFilter('store_id', Mage::app()->getStore()->getId())->getFirstItem();
  }
